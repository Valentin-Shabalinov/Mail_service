...
class Client(models.Model):
    ...
    middle_name = models.CharField(
        max_length=20, verbose_name="отчество", **NULLABLE
    )
...
class Mailing(models.Model):
    ...
    regularity = models.CharField(
        max_length=10, choices=REGULARITY_CHOICES, verbose_name="периодичность"
    )
...
class MailingLog(models.Model):
    ...
    timestamp = models.DateTimeField(
        default=datetime.now, verbose_name="дата и время попытки"
    )
    status = models.CharField(
    ...
    client = models.ManyToManyField(
        Client, related_name="logs", verbose_name="клиент"
    )
